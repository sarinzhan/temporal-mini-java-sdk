...
### 2. Схема БД
Движок хранит всё в схеме `wflow` и создаёт её сам через **Flyway** при старте приложения — ничего
применять руками не нужно. Миграции лежат в `db/migration/wflow` (внутри артефакта), с отдельной
таблицей истории `wflow.flyway_schema_history`, поэтому они не конфликтуют с миграциями вашего
приложения. Boot прогоняет их до инициализации JPA, так что таблицы готовы раньше любого обращения к БД.

Для существующих БД (таблицы уже созданы прежним `schema.sql`) включён `baselineOnMigrate` — Flyway
просто пометит схему как версию V1 и не будет пересоздавать таблицы. Если схемой управляете вы сами —
отключите: `spring.flyway.enabled=false`.
